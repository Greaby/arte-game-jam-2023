Shader "Hidden/SC Post Effects/Lensflares"
{
	...
	float4 FragGhosting(Varyings input) : SV_Target
	{
		...
		//Add color ramp
		float4 colorRamp = SAMPLE_TEXTURE2D(_ColorTex, Clamp, length(centerVec - texcoord) / 0.70).rgba;
	...
	float4 FragBlend(Varyings input) : SV_Target
	{
		...
		float3 flares = SAMPLE_TEXTURE2D(_FlaresTex, sampler_FlaresTex, UV).rgb;
		//return float4(flares.rgb, 1);
		float mask = SAMPLE_TEXTURE2D(_MaskTex, Clamp, UV).r;
		...
		if(_ChromaticAbberation > 0)
		{
			...
			float red = SAMPLE_TEXTURE2D(_FlaresTex, Clamp, UV + direction * distortion.r).r;
			float green = SAMPLE_TEXTURE2D(_FlaresTex, Clamp, UV + direction * distortion.g).g;
			float blue = SAMPLE_TEXTURE2D(_FlaresTex, Clamp, UV + direction * distortion.b).b;
	...
	float4 FragDebug(Varyings input) : SV_Target
	{
		...
		float3 flares = SAMPLE_TEXTURE2D(_FlaresTex, Clamp, UV).rgb;
		float mask = SAMPLE_TEXTURE2D(_MaskTex, Clamp, UV).r;
