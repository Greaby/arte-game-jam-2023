...
namespace SCPE
{
    ...
    public sealed class LensFlares : PostProcessEffectSettings
    {
        ...
        private void Reset()
        {
            ...
            if (colorTex.value == null)
            {
                colorTex.overrideState = true;
                colorTex.value = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>(UnityEditor.AssetDatabase.GUIDToAssetPath("af4273a5206164b4fa126b91bc0f5e78"));
            }
            if (maskTex.value == null)
            {
                maskTex.overrideState = true;
                maskTex.value = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>(UnityEditor.AssetDatabase.GUIDToAssetPath("b0860ff85f5d4e040b7596fafe2d7c8f"));
            }
            #endif
        }
