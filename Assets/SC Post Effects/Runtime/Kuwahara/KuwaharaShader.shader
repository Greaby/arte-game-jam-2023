Shader "Hidden/SC Post Effects/Kuwahara"
{
		...
		for (k = 0; k < 4; ++k) {
			...
			for (int j = W[k].y1; j <= W[k].y2; ++j) {
				for (int i = W[k].x1; i <= W[k].x2; ++i) {
					...
					float3 c = SAMPLE_TEXTURE2D_X_LOD(_MainTex, Clamp, uv + float2(i * _MainTex_TexelSize.x * 1, j * _MainTex_TexelSize.y), 0).rgb;
					m[k] += c;
					s[k] += c * c ;
				}
			}
	...
	float4 FragDepthAware(Varyings input) : SV_Target
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
