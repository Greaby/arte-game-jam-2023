Shader "Hidden/SC Post Effects/Tube Distortion"
{
	...
	float4 FragBuldge(Varyings input) : SV_Target
	{
		...
		float2 uv = BuldgedUV(UV, _Amount, lerp(1, 0.75, _Amount));
	...
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
		float2 uv = PinchUV(UV);

		float2 blackEdge = 1-ceil((uv.xy -1) * (uv.xy / UV * 0.001)).rg;
	...
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
		float2 uv = lerp(UV, UV * EdgesUV(UV), _Amount);
