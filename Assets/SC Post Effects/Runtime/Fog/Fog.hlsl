...
float4 GetFogColor(float2 screenPos, float3 worldPos, float distanceFactor)
{
    ...
    {
        fogColor = SAMPLE_TEXTURE2D(_ColorGradient, Clamp, float2(distanceFactor / _FarClippingPlane, 0));
    }
#endif
#ifdef ENABLE_SKYBOXCOLOR
    if (_SceneFogMode.z == 2) //Skybox
    {
        fogColor = SAMPLE_TEXTURE2D_X_LOD(_SkyboxTex, Clamp, screenPos, 0).rgba; //Mip not used
    }
#endif
...
float GetFogHeight(float3 worldPos, float time, float skyboxMask)
{
    ...
    if (_DistanceParams.w == 1) { //Heightfog enabled
        ...
        {
            float noise1 = SAMPLE_TEXTURE2D(_NoiseTex, Repeat, worldPos.xz * _NoiseParams.x + (time * _NoiseParams.y * float2(0, 1))).r;
            float noise2 = SAMPLE_TEXTURE2D(_NoiseTex, Repeat, worldPos.xz * _NoiseParams.x * 0.5 + (time * _NoiseParams.y * 0.8 * float2(0, 1))).r;
