Shader "Hidden/SC Post Effects/3D Hue Shift"
{
	...
	//z: Geometry influence
	//w: Intensity
	float4 FragSpectrum(Varyings input): SV_Target
	{
		...
		//Add waves
		noise = noise + RadialWave(UV * waveSize, float2(0.5, 0.5) * waveSize);
	...
	float4 FragGradientTex(Varyings input) : SV_Target
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
		...
		float3 hsv = float3(noise, 1, 1);
		//RGB
		float4 col = SAMPLE_TEXTURE2D(_GradientTex, Repeat, float2(noise, 0));
