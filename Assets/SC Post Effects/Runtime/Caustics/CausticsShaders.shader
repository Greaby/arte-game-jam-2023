Shader "Hidden/SC Post Effects/Caustics"
{
	...
	float4 Frag(Varyings input) : SV_Target
	{
		...
		float3 caustics1 = SAMPLE_TEXTURE2D(_CausticsTex, sampler_CausticsTex, uv + (_Time.y * float2(_CausticsParams.y, _CausticsParams.y))).rgb;
		float3 caustics2 = SAMPLE_TEXTURE2D(_CausticsTex, sampler_CausticsTex, (uv * 0.8) -(_Time.y * float2(_CausticsParams.y, _CausticsParams.y))).rgb;
