Shader "Hidden/SC Post Effects/Mosaic"
{
		...
		float4 FragTriangles(Varyings input) : SV_Target
		{
			UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);

			return SCREEN_COLOR(TrianglesUV(UV, _Params.x));
		}

		float4 FragHex(Varyings input) : SV_Target
		{
			UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);

			return SCREEN_COLOR(HexUV(HexIndex(UV, _Params.x)));
		}

		float4 FragCircle(Varyings input) : SV_Target
		{
			UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
